...
(function(){
  // ===== pick current language (reuse ?lang / cookie if present) =====
  ...
  // ===== create Internet Tools block (DOM) =====
  function makeBlock(){
    ...
    const items = [
      ...
      { href:'/updatetool/networkandAI./tools/tools/tools/dns-check/index.html', title:L.dns,   desc:L.dnsDesc,   icon:'🧭' },
      { href:'/updatetool/networkandAI./tools/tools/tools/speed-test/index.html', title:L.speed, desc:L.speedDesc, icon:'⚡' }
    ];
  ...
  // ===== insert before "Time", else after "Converters", else end =====
  ...
  // ===== Easter eggs additions (non-intrusive) =====
  function setupEasterEggs(){
    // robustly find search input & form
    ...
    function showModalWithGame(gamePath){
      // Use existing modal if present
      let modal = document.getElementById('game-modal');
      let frame = document.getElementById('game-frame');
      let title = document.getElementById('game-title');
      let close = document.getElementById('game-close');
      if (!modal) {
        modal = document.createElement('div'); modal.id = 'game-modal'; modal.setAttribute('role','dialog'); modal.setAttribute('aria-modal','true');
        modal.innerHTML = '<div id="game-box" style="position:fixed;inset:10% 10%;background:#0c1426;border:1px solid #2a3c64;border-radius:14px;box-shadow:0 10px 40px rgba(0,0,0,.5);display:flex;flex-direction:column;z-index:9999">'+
                          '<div id="game-head" style="display:flex;align-items:center;justify-content:space-between;padding:10px 12px;border-bottom:1px solid #223357;color:#eaf3ff"><strong id="game-title">🎮 Mini Game</strong><button id="game-close" aria-label="Close" style="background:#12213c;border:1px solid #2a3c64;border-radius:8px;color:#eaf3ff;padding:4px 10px;cursor:pointer">×</button></div>'+
                          '<iframe id="game-frame" title="Game" style="flex:1;border:0;border-radius:0 0 14px 14px;background:#000"></iframe>'+
                          '</div>';
      ...
      frame.src = new URL(gamePath, location.origin + '/').href;
      modal.style.display = 'block';
    }

    function showGreenTopMessage(){
      const bar = document.createElement('div');
      bar.style.cssText = 'position:fixed;top:0;left:0;right:0;background:#0b7f3a;color:#eaffea;text-align:center;padding:14px 8px;z-index:10000;font-weight:800;font-size:22px;line-height:1.25;';
      bar.innerHTML = '我还没空做这个，<br/>欢迎下次莅临指导';
    ...
    function handler(e){
      ...
      // otherwise do nothing (keep existing eggs & search behavior)
      return true;
    }
  ...
  if (document.readyState === 'loading') {
    ...
    insertInternetTools(); setupEasterEggs();
  }
})();
